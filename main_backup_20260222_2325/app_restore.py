...
with tab1:
    ...
    # 분석하기 / 닫기 버튼 (결과 있을 때만 닫기 표시)
    has_result = st.session_state.get("show_ticker_analysis") and "ticker_result" in st.session_state
    ...
    with btn_col1:
        if st.button("분석하기", type="primary"):
            ...
            if "error" in res:
                ...
                st.session_state["show_ticker_analysis"] = True
                st.rerun()
    with btn_col2:
        if has_result and st.button("닫기"):
            st.session_state.pop("ticker_result", None)
            st.session_state.pop("show_ticker_analysis", None)
            st.rerun()
